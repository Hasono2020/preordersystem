...
// ── Helper: build grid map from variants array ────────────────────────────────
function variantsToGrid(variants: any[]): Record<string, number> {
    const grid: Record<string, number> = {};
    for (const v of variants ?? []) {
        grid[`${v.color}__${v.size}`] = v.quantity || 0;
    }
    return grid;
}
...
export default function ProductEdit({ product }: any) {
    ...
    // Initialise tags and grid from existing product data
    const [colors,  setColors]  = useState<string[]>(product.colors ?? []);
    const [sizes,   setSizes]   = useState<string[]>(product.sizes  ?? []);
    const [varGrid, setVarGrid] = useState<Record<string, number>>(
        variantsToGrid(product.variants ?? [])
    );
    ...
    function submit(e: React.FormEvent) { e.preventDefault(); patch(`/products/${product.id}`); }
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-0.5">
                                    ...
                                    {codeEndsWithZ && (
                                        <span className="ml-1 text-orange-600 font-medium">Code ends with Z — typically excluded.</span>
                                    )}
                                </p>
                            </div>
